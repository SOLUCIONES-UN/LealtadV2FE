...
const getDepartamento = () => {
    return new Promise((resolve, reject) => {
        ...
        fetch(`${url}Departamento`, requestOptions)
            ...
            .then((data) => {
                const result = data.departamentos;
                ...
                if (Array.isArray(result)) {
                    ...
                    // Añadir el event listener para el select de departamentos
                    $("#departamento").on("change", function() {
                        const selectedDeptoId = $(this).val();
                        if (selectedDeptoId) {
                            getMunicipioByDepto(selectedDeptoId);
                        }
                    });
                    ...
                    console.error("La respuesta no contiene un array de departamentos válido.");
                    reject(new Error("Error en la estructura de la respuesta"));
                }
            })
            .catch((error) => {
...
// Función para traer municipios según el departamento
const getMunicipioByDepto = (idDepartamento) => {
    return new Promise((resolve, reject) => {
        ...
        $('#municipio').empty().append('<option disabled selected>Selecciona una opción</option>');
        $('#municipioEdit').empty().append('<option disabled selected>Selecciona una opción</option>');
...
// Inicializar los departamentos cuando la página esté lista
$(document).ready(() => {
...
const getTransaccion = () => {
    return new Promise((resolve, reject) => {
        ...
        fetch(`${url}Transaccion`, requestOptions)
            .then((response) => response.json())
            .then((data) => {
                const result = data.Transaccion;

                if (Array.isArray(result)) {
                    $("#transaccion").empty();
                    $("#transaccionEdit").empty();
                    ...
                    console.log("Transacciones obtenidas:", datatransaccionView);

                    resolve();
                } else {
                    console.error("La respuesta no contiene un array de transacciones válido.");
                    reject(new Error("Error en la estructura de la respuesta"));
                }
            })
            .catch((error) => {
        ...
        fetch(`${url}Premio`, requestOptions)
            ...
            .then((data) => {
                ...
                if (Array.isArray(result)) {
                    ...
                    console.log("Premios obtenidos:", dataPremioView);

                    resolve(); // Resuelve la promesa cuando los datos se han cargado correctamente
                } else {
                    console.error("La respuesta no contiene un array de premios válido.");
                    reject(new Error("Error en la estructura de la respuesta"));
                }
            })
            .catch((error) => {
